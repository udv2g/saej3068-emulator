...
**WARNING:** This code is constantly being updated and improved bugs maybe present. 

---
## Recent Updates

- Fixed Bug which had EvId as Frame Identifier 16 instead of 15 and SeId as Frame Identifier 15 instead of 16 
- CRC 32 Implementation **WORK  IN PROGRESS** Checked  via Lin Control Pilot Parser
- Code generator updated for Python output to go into [Lin Control Pilot Parser](https://github.com/udv2g/Lin-Control-Pilot-Parser-and-Interpreter?tab=readme-ov-file#parts-list)
---
Usage
---
...
---
Included is a code generator written in [Octave](https://octave.org) script (open-source Matlab, the script uses constructs not supported by Matlab) to convert a template file and an annotated description of the SAE J3068/1 and J3068/2 data fields (`field table pub.ods`) to generate the multiplexing code (`ids_handler.c`).  See below for usage.
...
The function declaration of the top-level script is:
    ...
    function gen_field_code(ods_in, c_in, c_out, print_command='commented', rpage=false, 'CAPL', interactive=false)

| Argument        | Description                                                                                                                                                                                                                      |
|:----------------|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `slash`         | Input either `1` or `2` for the SAE J3068/1 interpretation or SAEJ3068/2 interpretation                                                                                                                                          |
...
| `CAPL`  (*new)  | Takes in `CAPL`, `C`, or `PYTHON` to generate that code into the output file.  Defaults to `CAPL`.                                                                                                                               |
...
After running `pkg load io` in the octave command window you run this example command below /1 call is below can do /2 as well.
```bash
gen_field_code(1,'field table pub CAPL.ods', 'ids-template.can', 'out.can', 'yes', false, 'CAPL', false)
```
...
After running `pkg load io` in the octave command window you run this example command below /1 call is below can do /2 as well.
```bash
gen_field_code(1,'field table pub.ods', '../ids_handler.c', 'out_c.c', 'yes', false, 'C', false)
```

### PYTHON Usage

After running `pkg load io` in the octave command window you run this example command below for /1 and /2 interpretation.
```bash
gen_field_code(1,'field table pub PYTHON.ods', 'ids-template-PY.py', 'out_python-1.py', 'yes', false, 'PYTHON', false)   
```
```bash
gen_field_code(2,'field table pub.ods', 'J3072-template-PYTHON.py', 'out_python-2.py', 'yes', false, 'PYTHON', false)  
```
This output is specifically meant to be used for generating code for the Lin Control Pilot Parser and Interpreter.